...
# ═════════════════════════════════════════════════════
# 1. IMPORT TEMA (dengan pengaman)
# ══════════════════════════════════════════════════════
try:
    from config.theme import apply_theme, theme_toggle
except ImportError:
    def apply_theme(): pass
    def theme_toggle(): pass
...
try:
    ...
    BASE_MODULES_LOADED = True
except ImportError as e:
    BASE_MODULES_LOADED = False
    st.error(f"⚠️ Modul dasar belum lengkap: {e}")

# ══════════════════════════════════════════════════════
# 4. IMPORT MODUL LANJUTAN (OPSIONAL - ANTI CRASH)
# ══════════════════════════════════════════════════════
try:
...
if menu == "🔍 Screener":
    ...
    if not BASE_MODULES_LOADED:
        st.error("Modul dasar belum siap.")
    else:
...
# ══════════════════════════════════════════════════════
    ...
    if not BASE_MODULES_LOADED:
        st.error("Modul Portfolio belum siap.")
    else:
...
# ══════════════════════════════════════════════════════
    ...
    if not BASE_MODULES_LOADED:
        st.error("Modul Journal belum siap.")
    else:
...
# ══════════════════════════════════════════════════════
    ...
    st.title("🔔 Alerts")
    if not BASE_MODULES_LOADED:
        st.error("Modul Alerts belum siap.")
    else:
        ...
        if st.button("🔍 Check Alerts Now", type="secondary"):
            ...
            if triggered:
                for t in triggered:
                    st.success(f"🚨 {t['ticker']} — {t['condition']} terpenuhi!")
            else:
                st.info("Tidak ada alert yang ter-trigger.")
...
# ══════════════════════════════════════════════════════
# MENU LANJUTAN (AI, PDF, FUNDAMENTAL, DLL) - ANTI CRASH
# ══════════════════════════════════════════════════════
elif menu in ["🤖 AI Summary", "📄 PDF Report", "📊 Fundamentals", "⏳ Multi-Timeframe", "📈 Backtest", "🌐 Discovery"]:
    st.title(menu)
    if not ADVANCED_MODULES_LOADED:
        st.warning(f"⚠️ Modul untuk **{menu}** belum tersedia.")
        st.info("""
        **Cara Memperbaiki:**
        Pastikan file-file berikut sudah ada:
        - `ai/summarizer.py`
        - `reports/pdf_generator.py`
        - `fundamentals/fetcher.py`
        - `timeframes/engine.py`
        - `backtest/engine.py`
        - `data/ticker_discovery.py`
        - `alerts/scheduler.py` & `alerts/notifier.py`
        
        Menu ini aman (tidak crash) sampai modul ditambahkan.
        """)
    else:
        st.success(f"Modul {menu} siap digunakan!")
...
# ══════════════════════════════════════════════════════
    ...
    if not BASE_MODULES_LOADED:
        st.error("Modul Data Manager belum siap.")
    else:
        ...
        with c2:
            if st.button("🧹 Clear Database"):
                st.info("Untuk Supabase, hapus data via Dashboard Supabase → Table Editor.")
...
# ══════════════════════════════════════════════════════
    ...
    if not BASE_MODULES_LOADED:
        st.error("Modul Export belum siap.")
    else:
        c1, c2 = st.columns(2)
        ...
        if st.button("📥 Download Excel (All Sheets)", type="primary"):
            ...
            st.download_button(label="Klik untuk Download", data=xlsx, file_name=f"Cuanku_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
