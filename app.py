...
if menu == "🔍 Screener":
    ...
    if not df_screen.empty:
        ...
        if not df_price.empty:
            ...
            # Add EMA20
            fig.add_trace(go.Scatter(x=df_price["date"], y=df_price["close"].ewm(span=20).mean(),
...
# ═══════════════════════════════════════════════════════
    ...
    if not wl.empty:
        ...
        wl["gap_to_target"] = wl.apply(
            lambda r: f"{((r['target_price'] - r['close'])/r['close']*100):.1f}%" if pd.notna(r.get("target_price")) and r.get("close") else "-", axis=1
        )
        st.dataframe(wl[["ticker", "notes", "target_price", "close", "gap_to_target", "added_date"]],
...
# ═══════════════════════════════════════════════════════
    ...
    df_pos = portfolio.get_df()
    if not df_pos.empty:
        st.dataframe(df_pos[["ticker", "name", "sector", "shares", "avg_price", "current_price",
                             "unrealized_pl", "market_value"]], use_container_width=True, hide_index=True)
    else:
